...
SET @sql_hm := IF(@has_cg = 0 OR @exist_hm > 0, 'SELECT 1',
...
PREPARE stmt FROM @sql_hm; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql_cts := IF(@has_cg = 0 OR @exist_cts > 0, 'SELECT 1',
...
PREPARE stmt FROM @sql_cts; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql_ctl := IF(@has_cg = 0 OR @exist_ctl > 0, 'SELECT 1',
...
PREPARE stmt FROM @sql_ctl; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @sql_upd := IF(@has_cg = 0, 'SELECT 1',
  'UPDATE class_groups SET homeroom_source = ''manual'' WHERE default_room_id IS NOT NULL AND (homeroom_source IS NULL OR homeroom_source = '''')');
