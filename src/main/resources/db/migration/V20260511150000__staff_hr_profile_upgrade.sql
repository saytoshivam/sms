...
-- Guard: staff is Hibernate-managed; wrap in stored procedure to skip on fresh DB.

DROP PROCEDURE IF EXISTS _sp_staff_hr_profile_upgrade;
CREATE PROCEDURE _sp_staff_hr_profile_upgrade()
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.TABLES
               WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'staff') THEN

        IF NOT EXISTS (SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'staff' AND COLUMN_NAME = 'staff_type') THEN
            ALTER TABLE staff
    ...
    END IF;
END;
CALL _sp_staff_hr_profile_upgrade();
DROP PROCEDURE IF EXISTS _sp_staff_hr_profile_upgrade;
