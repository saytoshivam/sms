...
DROP PROCEDURE IF EXISTS _mig_subject_venue_varchar;
CREATE PROCEDURE _mig_subject_venue_varchar()
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'subjects') THEN
    ...
    END IF;
END;
CALL _mig_subject_venue_varchar();
DROP PROCEDURE IF EXISTS _mig_subject_venue_varchar;