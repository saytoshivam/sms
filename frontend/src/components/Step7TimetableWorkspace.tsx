...
export default function Step7TimetableWorkspace({
  ...
  viewVersion = null,
  readOnly = false,
  /** When true, missing slots triggers one automatic POST generate-from-onboarding (wizard only). That API clears all timetable entries—never auto-run on the standalone module route. */
  autoGenerateSlotsFromOnboardingOnMount = false,
  surface = 'wizard',
}: {
  ...
  viewVersion?: { id: number; status: string; version: number } | null;
  readOnly?: boolean;
  autoGenerateSlotsFromOnboardingOnMount?: boolean;
  /** Full Step 7 shell (wizard) vs editor grid only (“Timetable Editor” card used on Academic → Timetable). */
  surface?: 'wizard' | 'module';
}) {
  ...
  useEffect(() => {
    if (!autoGenerateSlotsFromOnboardingOnMount) return;
    if (autoSlotsAttempted) return;
    if (setup.isLoading || setup.isError) return;
    if ((setup.data?.slots ?? []).length > 0) return;
    // Auto-generate once when user reaches onboarding Step 7 and slots are missing (not on /app/timetable — refresh would wipe entries).
  ...
  const upsertCell = useMutation({
    mutationFn: async (body: { dayOfWeek: string; timeSlotId: number; subjectId: number; staffId: number; roomId: number | null }) => {
      if (readOnly) throw new Error('Published timetable is read-only.');
      if (!versionId) throw new Error('Missing draft version');
              ...
              <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden w-full">
                  <div className="p-5 border-b border-slate-200">
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div>
                        <div className="text-sm font-black">{readOnly ? 'Timetable' : 'Timetable Editor'}</div>
                        <div className="mt-1 text-xs font-bold text-slate-500">
                          {readOnly ? 'Published schedule — view only.' : 'Switch tabs to refine by section, teacher, or room.'}
                        </div>
                      </div>
                      {readOnly ? null : (
                    ...
                    {slots.length ? (
                              ...
                              {days.map((d) => {
                                ...
                                return (
                                  <button
                                    ...
                                    type="button"
                                    onClick={() => {
                                      if (readOnly || tab !== 'SECTION') return;
                                      setDrawerKey(k);
                                    }}
                                    className={[
                                      ...
                                      tab !== 'SECTION' || readOnly ? 'cursor-default' : '',
                                    ].join(' ')}
                                  >
                      ...
                      <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4">
                        ...
                        <div className="mt-1 text-xs font-bold text-amber-800">
                          {readOnly
                            ? 'School periods are not configured in Basic Setup for this view.'
  ...
  if (surface === 'module') {
    return (
      <div className="w-full space-y-4">
        ...
        {!loading && !loadError ? timetableEditorCard : null}
        {cellDrawer}
      </div>
    );
  }
