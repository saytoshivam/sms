...
function thisAcademicYearDefault() {
  const now = new Date();
  const y = now.getFullYear();
  // India typical: Apr–Mar; if Jan–Mar treat as previous year start
...
export function SchoolOnboardingWizardPage() {
  ...
  const [academicYear, setAcademicYear] = useState(() => thisAcademicYearDefault());
  const [startMonth, setStartMonth] = useState(4);
  const [workingDays, setWorkingDays] = useState<string[]>(['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT']);
  const [attendanceMode, setAttendanceMode] = useState<'DAILY' | 'LECTURE_WISE'>('LECTURE_WISE');
  // time slots are auto-generated later for timetable using open timings + lecture duration
  ...
  useEffect(() => {
    if (activeStepIndex !== 2) return;
    const rows = pageContent(subjectsCatalog.data);
    if (!rows.length) return;
    // Best-effort prefetch so delete can be disabled without waiting for a click.
    (async () => {
      for (const s of rows.slice(0, 80)) {
        if (!s?.id) continue;
        if (subjectDeleteInfoCache[s.id]) continue;
        try {
          const info = (await api.get<{ canDelete: boolean; reasons: string[] }>(`/api/subjects/${s.id}/delete-info`)).data;
          setSubjectDeleteInfoCache((p) => ({ ...p, [s.id]: info }));
        } catch {
          // ignore; keep delete enabled, backend will still enforce safety.
        }
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeStepIndex, subjectsCatalog.data]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeStepIndex, roomsSaved.data]);
  ...
  useEffect(() => {
    ...
    if (!d) return;
    setAcademicYear(d.academicYear || thisAcademicYearDefault());
  ...
  const saveBasic = useMutation({
    mutationFn: async () =>
      api.put('/api/v1/onboarding/basic-info', {
        ...
        <div className="row" style={{ gap: 12, flexWrap: 'wrap' }}>
          ...
          <div className="stack" style={{ flex: '1 1 220px' }}>
            ...
            <SelectKeeper
              ...
              onChange={(v) => setStartMonth(Number(v || 4))}
          ...
          <div className="row" style={{ gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            <div className="stack" style={{ minWidth: 260 }}>
              <SelectKeeper
                ...
                onChange={(v) => setAttendanceMode(((v || 'LECTURE_WISE') as 'DAILY' | 'LECTURE_WISE') ?? 'LECTURE_WISE')}
                options={[
            ...
            <div className="stack" style={{ minWidth: 220 }}>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (v === '') setLectureDurationMinutes('');
                  else setLectureDurationMinutes(Number(v));
                }}
                placeholder="45"
              />
            </div>
      ...
      <div className="card stack" style={{ gap: 14 }}>
        ...
        <div className="row" style={{ gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <span className="muted" style={{ fontSize: 12 }}>
            Search filters both queued and saved lists
          </span>
        </div>
      ...
      <div className="card stack" style={{ gap: 14 }}>
        ...
        <div className="stack" style={{ gap: 10, marginTop: 6 }}>
            ...
            <div className="row" style={{ gap: 10 }}>
              ...
              <div style={{ minWidth: 200 }}>
                <SelectKeeper
                  ...
                  onChange={(v) => setSavedRoomsTypeFilter(((v || 'ALL') as any) ?? 'ALL')}
                  options={[
          ...
          {(() => {
            ...
            if (rows.length === 0) return <div className="muted">No rooms saved yet.</div>;
                      ...
                      <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                        <div className="row" style={{ gap: 10 }}>
                          <button
                            ...
                            onClick={() => setSavedRoomsExpandedBuildings((p) => ({ ...p, [building]: !(p[building] ?? true) }))}
                          >
                            {bOpen ? 'Hide' : 'Show'}
                          </button>
                                ...
                                <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                                  <div className="row" style={{ gap: 10 }}>
                                    <button
                                      ...
                                      onClick={() => setSavedRoomsExpandedFloors((p) => ({ ...p, [k]: !(p[k] ?? true) }))}
                                    >
                                      {fOpen ? 'Hide' : 'Show'}
                                    </button>
                                    <div className="muted" style={{ fontWeight: 900 }}>
        ...
        <ConfirmDialog
          ...
          details={[
            'Changes apply immediately. Lab type is only applicable when Type is LAB.',
          ]}
              ...
              >
                ...
                <div className="stack" style={{ gap: 6, flex: '1 1 220px' }}>
                  ...
                  <SelectKeeper
                    ...
                    onChange={(v) => setStaffTableLogin((v as any) ?? 'ALL')}
                    options={[
                      { value: 'ALL', label: 'All' },
